    ...
    ******************/
    constructor(
        ...
        if (_summoningDeposit > 0) {
            ...
            unsafeAddToBalance(GUILD, _depositToken, _summoningDeposit);
        }
        ...
        for (uint256 i = 0; i < _summoners.length; i++) {
            ...
            totalShares = totalShares + _summonerShares[i];
        }
    ...
    function makeSummoningTribute(uint256 tribute) payable external {
        ...
        if (depositToken == wETH && msg.value > 0) {
            ...
            require(IERC20(depositToken).transferFrom(msg.sender, bank, tribute), "transfer failed");
        }
...
contract MolochSummoner {
    ...
    event SummonMoloch(address indexed moloch, address[] summoners, address indexed depositToken, uint256 summoningTime, uint256 periodDuration, uint256 votingPeriodLength, uint256 gracePeriodLength, uint256 proposalDeposit, uint256 dilutionBound, uint256 processingReward, uint256[] summonerStake, uint256 summoningDeposit, uint256 summoningRate, uint256 summoningTermination);
